  ...
  try {
    ...
    if (cohort.length > 0) {
      ...
      res
        .status(404)
        .json({
          error: `A cohort with the ID of ${req.params.id} does not exist.`
        });
    }
  } catch (error) {
    const errorMsg = errors[error.errno] || errors[default_error];
  ...
  try {
    ...
    if (students.length > 0) {
      ...
      res
        .status(404)
        .json({
          error: `The cohort with the ID of ${
            req.params.id
  ...
  try {
    ...
    if (cohort > 0) {
      ...
      res
        .status(404)
        .json({
          error: `The cohort with the ID of ${req.params.id} does not exist.`
        });
    }
  } catch (error) {
    const errorMsg = errors[error.errno] || errors[default_error];
